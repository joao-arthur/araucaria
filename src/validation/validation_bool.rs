...
mod tests {
    ...
    const OP_VALUE_EQ: Operation = Operation::Eq(Operand::Value(OperandValue::Bool(false)));
    const OP_VALUE_NE: Operation = Operation::Ne(Operand::Value(OperandValue::Bool(false)));
    const OP_VALUE_GT: Operation = Operation::Gt(Operand::Value(OperandValue::Bool(false)));
    const OP_VALUE_GE: Operation = Operation::Ge(Operand::Value(OperandValue::Bool(false)));
    const OP_VALUE_LT: Operation = Operation::Lt(Operand::Value(OperandValue::Bool(false)));
    const OP_VALUE_LE: Operation = Operation::Le(Operand::Value(OperandValue::Bool(false)));
    const OP_VALUE_BTWN: Operation = Operation::Btwn(Operand::Value(OperandValue::Bool(false)), Operand::Value(OperandValue::Bool(true)));
    ...
    const OP_FIELD_BTWN: LazyLock<Operation> =
    ...
    fn bool_validation() {
        ...
        assert_eq!(BoolValidation::default().eq(false), BoolValidation { required: true, operation: Some(OP_VALUE_EQ) });
        assert_eq!(BoolValidation::default().ne(false), BoolValidation { required: true, operation: Some(OP_VALUE_NE) });
        assert_eq!(BoolValidation::default().gt(false), BoolValidation { required: true, operation: Some(OP_VALUE_GT) });
        assert_eq!(BoolValidation::default().ge(false), BoolValidation { required: true, operation: Some(OP_VALUE_GE) });
        assert_eq!(BoolValidation::default().lt(false), BoolValidation { required: true, operation: Some(OP_VALUE_LT) });
        assert_eq!(BoolValidation::default().le(false), BoolValidation { required: true, operation: Some(OP_VALUE_LE) });
        assert_eq!(BoolValidation::default().btwn(false, true), BoolValidation { required: true, operation: Some(OP_VALUE_BTWN) });
        assert_eq!(BoolValidation::default().eq_field(FIELD.into()), BoolValidation { required: true, operation: Some(OP_FIELD_EQ.clone()) });
        assert_eq!(BoolValidation::default().ne_field(FIELD.into()), BoolValidation { required: true, operation: Some(OP_FIELD_NE.clone()) });
        assert_eq!(BoolValidation::default().gt_field(FIELD.into()), BoolValidation { required: true, operation: Some(OP_FIELD_GT.clone()) });
        assert_eq!(BoolValidation::default().ge_field(FIELD.into()), BoolValidation { required: true, operation: Some(OP_FIELD_GE.clone()) });
        assert_eq!(BoolValidation::default().lt_field(FIELD.into()), BoolValidation { required: true, operation: Some(OP_FIELD_LT.clone()) });
        assert_eq!(BoolValidation::default().le_field(FIELD.into()), BoolValidation { required: true, operation: Some(OP_FIELD_LE.clone()) });
        assert_eq!(BoolValidation::default().btwn_field(FIELD.into(), FIELD_B.into()), BoolValidation { required: true, operation: Some(OP_FIELD_BTWN.clone()) });
    }
}
